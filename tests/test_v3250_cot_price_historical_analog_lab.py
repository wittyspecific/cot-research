...
def test_navigation_places_analog_then_fx_between_macro_and_market_regime():
    text = APP.read_text(encoding="utf-8")
    research = _research_items(text)

    macro = next(
        i for i, item in enumerate(research)
        if "pages/macro_model_library.py" in item
    )
    analog = next(
        i for i, item in enumerate(research)
        if "pages/cot_price_analog.py" in item
    )
    fx_analog = next(
        i for i, item in enumerate(research)
        if "pages/fx_relative_cot_analog.py" in item
    )
    market = next(
        i for i, item in enumerate(research)
        if "pages/market_regime.py" in item
    )
