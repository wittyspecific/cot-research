...
def test_existing_active_market_cannot_regress_to_planned_before_first_h1(monkeypatch):
    ...
    monkeypatch.setattr(
        tracker,
        "history_batch",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("no completed H1 -> no history request")),
    )
    monkeypatch.setattr(
        tracker,
    ...
    assert out["entry_time_utc"] == "2026-08-17T00:18:00Z"
    assert out["mfe_r"] == 0.4
    assert result["state_regressions_blocked"] == 1
